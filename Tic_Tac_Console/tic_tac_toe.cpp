...
const int WAY_TO_WIN[8][3] = { {0, 1, 2},
                         {0, 3, 6},
                         {1, 4, 7},
                         {2, 5, 8},
                         {0, 4, 8},
...
void print_board(){// for printing the board
cout<<endl;
cout<<endl<<"                                   |-----|-----|-----|";
...
void check_winner(){ // for checking if there's a winner
for (int i = 0; i < 8; ++i){
...
void ask_player_names(){// ask for players' names

cout<<"Who Would Be Player No.1, Pls. Enter Your Name: ";
getline(cin, PLAYER1);
cout<<"Who Would Be Player No.2, Pls. Enter Your Name: ";
getline(cin, PLAYER2);
cout<<endl<<"Good Luck to Both Players.";
cout<<endl<<PLAYER1<<" Will Be Using 'x' while "<<PLAYER2<<" Will Be Using 'o'";
    ...
    for(;;){//for players move
        if (whose_turn){
            ...
            if (MOVE < 9 && MOVE > -1 && BOARD[MOVE] != X_CHAR && BOARD[MOVE] != O_CHAR){
                BOARD[MOVE] = X_CHAR;
                break;
                }else cout<<endl<<"Invalid Input, Pls Try Again";
     ...
     check_winner();
     if (FINISH_OR_NOT){
            if (whose_turn == true)cout<<endl<<PLAYER1<<" Wins"<<endl;
